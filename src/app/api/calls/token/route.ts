...
import { getCallSessionToken } from '@/lib/phoneAFriendService';
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const sessionData = await getCallSessionToken({ callId, requesterId });
    return NextResponse.json(sessionData, { headers });
  } catch (error: any) {
    console.error('[API/calls/token POST]', error);
